...
#                linker = guess_nix_linker(env, compiler, cls, version, for_machine)
            ...
            def _get_linker_try_windows(cls: T.Type['Compiler']) -> T.Optional['DynamicLinker']:
                ...
                if 'windows' in out or env.machines[for_machine].is_windows():
                    ...
                    try:
                        ...
                        pass
                if linker is None:
                    linker = ZOSDynamicLinker(env, compiler, cls,
                                              version, for_machine)
                return linker
            ...
            if 'Open64 Compiler Suite' in err or 'xlf' in err:
                cls = fortran.Open64FortranCompiler
                env.coredata.add_lang_args(cls.language, cls, for_machine, env)
                linker = linkers.ZOSDynamicLinker(compiler, for_machine, cls.LINKER_PREFIX, [], version=version)
                return cls(
                    compiler, version, for_machine, is_cross, info,
                    full_version=full_version, linker=linker)
